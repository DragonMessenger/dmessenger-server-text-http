...
@app.route('/api/<path:channel_id>/pins/add/<path:message_id>', methods=['POST'])
async def channel_messages_pins_add(channel_id, message_id):
    ...
    cursor.execute(f"UPDATE messages SET is_pinned = 1 WHERE message_id = '{message_id}' AND channel_id = '{channel_id}'")
